...
public class GenresFlippedActivity extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sharedPreferences.edit().putBoolean("GENRES_FLIPPED_ACTIVITY_VISIBLE", true).commit();
	...
	private void inflateFragment() {
		...
		String songSource = getIntent().getExtras().getString("SONG_SOURCE");
		String songFilePath = getIntent().getExtras().getString("SONG_FILE_PATH");
		String albumArtPath = getIntent().getExtras().getString("ALBUM_ART_PATH");
		this.genreName = genreName;
		
		//Inflate the fragment within the activity layout.
	    final Fragment genresFlippedFragment = new GenresFlippedFragment();
	    Bundle bundle = new Bundle();
	    bundle.putString("GENRE_NAME", genreName);
	    bundle.putString("SONG_SOURCE", songSource);
	    bundle.putString("SONG_FILE_PATH", songFilePath);
	    bundle.putString("ALBUM_ART_PATH", albumArtPath);
	    genresFlippedFragment.setArguments(bundle);

	    FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
	    transaction.replace(R.id.artists_flipped_layout, genresFlippedFragment);
	    transaction.commit();
    ...
    public void onDestroy() {
    	super.onDestroy();
    	sharedPreferences.edit().putBoolean("GENRES_FLIPPED_ACTIVITY_VISIBLE", false).commit();
    ...
    public void onPause() {
    	...
    	if (isFinishing()) {
    		if (GenresFlippedListViewAdapter.musicLibraryDBHelper!=null) {
    			GenresFlippedListViewAdapter.musicLibraryDBHelper.close();
    			GenresFlippedListViewAdapter.musicLibraryDBHelper = null;
    		}
    		
    		if (GenresFlippedListViewAdapter.cursor!=null) {
    			GenresFlippedListViewAdapter.cursor.close();
    			GenresFlippedListViewAdapter.cursor = null;
    		}
