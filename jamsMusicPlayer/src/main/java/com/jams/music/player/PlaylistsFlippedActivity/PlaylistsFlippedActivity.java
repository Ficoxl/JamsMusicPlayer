...
public class PlaylistsFlippedActivity extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
    	...
    	setContentView(R.layout.activity_playlists_flipped);
	...
	private void inflateFragment() {
		String playlistName = getIntent().getExtras().getString("PLAYLIST_NAME");
		String playlistFilePath = getIntent().getExtras().getString("PLAYLIST_FILE_PATH");
		String playlistFolderPath = getIntent().getExtras().getString("PLAYLIST_FOLDER_PATH");
		String playlistSource = getIntent().getExtras().getString("PLAYLIST_SOURCE");
		String playlistId = getIntent().getExtras().getString("PLAYLIST_ID");
		this.playlistName = playlistName;
				
		//Check if we're dealing with one of the default playlists.
		if (!getIntent().getExtras().getString("DEFAULT_PLAYLIST_TYPE").equals("USER_PLAYLIST")) {
			//Yep, this is a default playlist.
			final Fragment defaultPlaylistFragment = new SmartPlaylistFragment();
			Bundle bundle = new Bundle();
			bundle.putString("DEFAULT_PLAYLIST_TYPE", getIntent().getStringExtra("DEFAULT_PLAYLIST_TYPE"));
			defaultPlaylistFragment.setArguments(bundle);
    	    FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
    	    transaction.replace(R.id.artists_flipped_layout, defaultPlaylistFragment);
    	    transaction.commit();
			
		} else {
    		//Nope, not a default playlist.
    	    final Fragment playlistsFlippedFragment = new PlaylistsFlippedFragment();
    	    Bundle bundle = new Bundle();
    	    bundle.putString("PLAYLIST_NAME", playlistName);
    	    bundle.putString("PLAYLIST_FILE_PATH", playlistFilePath);
    	    bundle.putString("PLAYLIST_FOLDER_PATH", playlistFolderPath);
    	    bundle.putString("PLAYLIST_SOURCE", playlistSource);
    	    bundle.putString("PLAYLIST_ID", playlistId);
    	    playlistsFlippedFragment.setArguments(bundle);

    	    FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
    	    transaction.replace(R.id.artists_flipped_layout, playlistsFlippedFragment);
    	    transaction.commit();
    ...
    public void onDestroy() {
    	super.onDestroy();
    	sharedPreferences.edit().putBoolean("PLAYLISTS_FLIPPED_ACTIVITY_VISIBLE", false).commit();
